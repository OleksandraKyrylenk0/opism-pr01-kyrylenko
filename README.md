...
|---|---|
| ;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: 42636| рядок який містить службову інформацію |
...
> На таку кількість груп мене наштовхнуло те, що виводи команд логічно діляться за своїм прямим призначенням на окремі етапи: від пошуку IP-адреси та з'єднання до шифрування, заголовків і самого HTML-коду.Рішення змінилося якби у виводі не було певних даних (наприклад, IP-адрес чи інфо про TLS-шифрування)

**D.3. Питання, яке залишилося без відповіді**

> На слайді 16 лекції №1 було показано, що є ще MAC-адреса, але її не було в жодному моєму виводі,чому? І де вона використовується?
---
...
|---|---|---|
| 1 | Частина B (Рядки, які не вдалося віднести до жодної групи, а точніше причини утруднення) | Що це за рядок такий? `;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: 42636` |
| 2 | Контрольні питання №1 | Поясни це питання більш зрозуміліше:<br>Скільки рядків діагностичного виводу передує отриманню даних сторінки (завдання A.1) |
...
|---|---|---|---|
| 1 | Перевірено,що це дійсно рядок який містить службову інформацію | зменшено та переписано речення у моєму розумінні|великий технічний опис,записала тільки суть |
| 2 | Перевірено чи вірно ШІ пояснив суть питання|перераховано рядки вручну у своєму виводі,щоб написати точне число |відхилено приклад від ШІ,бо у мене свої дані |
| 3 | Перевірено чи підходить відповідь для мого сайту| змінено вручну текст таблиці | відхилено варіант замінити знаки < та > на &lt; та &gt; бо це працювало некоректно|
...
> Довелося кілька разів дивитися як гарно зробити таблицю з пробілами.Виникли труднощі з командами &lt; &gt; та <code> бо це працювало не так як я хотіла.|
